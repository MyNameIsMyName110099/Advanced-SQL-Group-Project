...
There is a folder per person and a folder per week.

```
Kelsey/                each person's own scripts, exactly as they wrote them
Mason/
Nathan/
Nicholas/
...
The week folders hold what the group turns in - the combined script that builds everything
in one pass, the inserts, and the AI prompt document. That combined script is the one to
run on a fresh VM.

The person folders hold each of our own scripts as we wrote them. They are the record of
who did what, so they get left alone even after the combined script has moved past them.
Do not run those on your VM expecting a working database; run the week folder instead.
