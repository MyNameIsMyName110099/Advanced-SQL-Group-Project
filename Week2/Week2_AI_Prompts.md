...
> Here is the schema for our Restaurant database, eighteen tables across four
> groups. Write INSERT statements to populate every table.
>
...
> accented letters, because these scripts get pasted into SSMS on a virtual
> machine through a browser and non-ASCII characters get mangled in transit.
> Apostrophes in surnames like O'Brien need to be doubled for SQL.
>
...
### Prompt 6 - validating the generated SQL before running it

> Before these run against SQL Server, check the generated inserts against the
> schema and report anything that would be rejected:
>
...
> Two problems turned up while the files were being run.
>
...
>
> Then extend the validation to compare every insert's column list against the
> CREATE TABLE statements, so a column that does not exist on a table cannot get
> through again.

This was the gap in the earlier validation. It compared values against the
constraints but never confirmed that the column names themselves existed, which
is why the mismatch was not caught until SQL Server rejected it. The check now
runs across all eighteen tables.
...
The files that are submitted are ordinary T-SQL. Nothing outside SQL Server is
needed to run them.
